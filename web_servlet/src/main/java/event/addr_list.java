...
public class addr_list extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	Connection con = null;
	PreparedStatement ps = null;
	ResultSet rs = null;	//ResultSet => DML중에서 select 에서만 사용함
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			this.alldata = new ArrayList<ArrayList<String>>();	//jsp로 전달할 2차 배열
			while(this.rs.next()) {
				this.data = new ArrayList<String>();	//1차 클래스를 생성하여 2차 배열에 적용
				//Database에서 사용하는 컬럼명을 순서에 맞게 적용
				...
				this.alldata.add(this.data);	//2차 클래스배열에 추가
			}
			System.out.println(this.alldata);
		} catch (Exception e) {
			System.out.println("DB서버 오류로 인하여 원활한 서비스 불가능!!");
		} finally {
			try {
				this.ps.close();
				this.con.close();
		...
		request.setAttribute("addr", this.alldata);	//jsp로 2차배열 전달
